...
import { OriginFields } from './fields';
import { originSchema, OriginFormData } from './schema';

const adapter = makeLocalStorageAdapter<Origin>('erp.master.origin');
...
export function OriginPage() {
  ...
  const [editingData, setEditingData] = useState<Origin | undefined>();
  ...
  const handleEdit = async (id: string) => {
    try {
      ...
      toast.error('Failed to load origin');
    }
  };

  const handleSave = async (data: Omit<OriginFormData, 'id'|'createdAt'|'updatedAt'>) => {
    setIsLoading(true);
    try {
      if (editingId) {
  ...
  const columns = [
    ...getDefaultColumns<Origin>(),
    {
      key: 'address' as keyof Origin,
      header: 'Address',
      render: (row: Origin) => (
        <span className="max-w-xs truncate block" title={row.address || ''}>
          {row.address || '-'}
        </span>
      )
    }
  ...
  return (
    <div className="p-6">
      ...
      <MasterForm<OriginFormData>
        ...
        schema={originSchema}
        renderExtra={({ form }) => <OriginFields form={form} />}
        isLoading={isLoading}
      />
    </div>
  );
