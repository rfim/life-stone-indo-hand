...
import { CustomerTypeFields } from './fields';
import { customerTypeSchema, CustomerTypeFormData } from './schema';
...
export function CustomerTypePage() {
  ...
  const [editingData, setEditingData] = useState<CustomerType | undefined>();
  ...
  const handleEdit = async (id: string) => {
    try {
      ...
      console.error('Failed to load customer type:', error);
      toast.error('Failed to load customer type');
    }
  };

  const handleSave = async (data: Omit<CustomerTypeFormData, 'id'|'createdAt'|'updatedAt'>) => {
    setIsLoading(true);
    try {
      if (editingId) {
  ...
  const columns = [
    ...getDefaultColumns<CustomerType>(),
    {
      key: 'description' as keyof CustomerType,
      header: 'Description',
  ...
  return (
    <div className="p-6">
      ...
      <MasterForm<CustomerTypeFormData>
        title="Customer Type"
        open={isSheetOpen}
        isEdit={!!editingId}
        initial={editingData}
        ...
        renderExtra={({ form }) => <CustomerTypeFields form={form} />}
        isLoading={isLoading}
      />
    </div>
  );
