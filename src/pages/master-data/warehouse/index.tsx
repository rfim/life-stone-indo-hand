...
import { warehouseSchema, WarehouseFormData } from './schema';

const adapter = makeLocalStorageAdapter<Warehouse>('erp.master.warehouse');
...
export function WarehousePage() {
  ...
  const [editingData, setEditingData] = useState<Warehouse | undefined>();
  ...
  const handleSave = async (data: Omit<WarehouseFormData, 'id'|'createdAt'|'updatedAt'>) => {
    setIsLoading(true);
    try {
      if (editingId) {
  ...
  const columns = [
    ...getDefaultColumns<Warehouse>(),
    {
      key: 'location' as keyof Warehouse,
      header: 'Location',
      render: (row: Warehouse) => (
        <span className="max-w-xs truncate block" title={row.location || ''}>
          {row.location || '-'}
        </span>
      )
    }
  ...
  return (
    <div className="p-6">
      ...
      <MasterForm<WarehouseFormData>
        ...
        renderExtra={({ form }) => <WarehouseFields form={form} />}
        isLoading={isLoading}
      />
    </div>
  );
